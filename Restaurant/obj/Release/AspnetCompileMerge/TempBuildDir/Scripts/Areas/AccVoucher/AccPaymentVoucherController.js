angular.module('myApp').controller('AccPaymentVoucherController', function ($scope, $window, $http, $uibModal, Upload, $location, $routeParams, $filter, $route) {

    var today = new Date();
    $scope.PaymentVoucherEntry = {
    ...
    $scope.GetPaymentVoucherNumber = function () {
        //$scope.VNumber = [];
        $http.get('/AccVoucher/GetPaymentVoucherNumber/').
          success(function (data) {
              if (data.success) {
                  $scope.PaymentVoucherEntry.VNumber = data.result;
              }
              else {
    ...
    $scope.GetPaymentLadgerList = function () {
        $scope.LadgerList = [];
        $http.get('/AccVoucher/GetPaymentLadgerList/').
          success(function (data) {
              if (data.success) {
    ...
    $scope.SavePaymentVoucher = function () {
        //if ($scope.AccPaymentVoucherForm.$valid) {
            ...
            if ($scope.TotalDebit === $scope.TotalCredit && $scope.TotalCredit !== 0 && $scope.TotalDebit !== 0) {
                $http({
                    method: 'POST',
                    url: '/AccVoucher/SavePaymentVoucher/',
                    data: { VNumber: $scope.PaymentVoucherEntry.VNumber, Narration: $scope.PaymentVoucherEntry.Narration, VoucherDetails: $scope.choices }
                }).success(function (data) {
                    if (data.success) {
    ...
    $scope.GetAllPaymentVoucherList = function () {
        $scope.PaymentVoucherEntryList = [];
        $http.get('/AccVoucher/GetAllPaymentVoucherList/').
          success(function (data) {
              if (data.success) {
                  $scope.PaymentVoucherEntryList = data.result;
              }
              else {
                  toastr.error(data.errorMessage);
              }
    ...
    $scope.EditPaymentVoucher = function (x) {
        $http({
            method: 'POST',
    ...
    $scope.UpdatePaymentVoucher = function () {
        //if ($scope.AccPaymentVoucherForm.$valid) {
            $http({
                method: 'POST',
                url: '/AccVoucher/UpdatePaymentVoucher/',
                data: { PaymentVoucherEntry: $scope.PaymentVoucherEntry, PaymentVoucherDetails: $scope.choices }
            }).success(function (data) {
                if (data.success) {
                    toastr.success(data.successMessage);
                    $scope.AddButton = true;
