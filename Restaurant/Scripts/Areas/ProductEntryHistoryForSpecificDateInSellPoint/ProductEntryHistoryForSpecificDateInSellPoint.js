angular.module('myApp').controller('ProductEntryHistoryForSpecificDateInSellPointController', function ($scope, $window, $http, $uibModal, Upload, $location, $routeParams, $filter, $route) {
    ...
    $scope.GetSellsPointInformation = function () {
        ...
        $http.get('/ProductEntryHistoryForSpecificDateInSellPoint/GetSellsPointInformation/').
    ...
    $scope.SearchProductTransactionList = function () {
        if ($scope.ProductEntryHistoryForSpecificDateInSellPointForm.$valid) {
            $http({

                method: 'POST',
                url: '/ProductEntryHistoryForSpecificDateInSellPoint/SearchProductTransactionList/',
                data: { sellsPointStoreId: $scope.SellsPoint.SellsPointStoreId, shiftId: $scope.SellsPoint.ShiftId, fromDate: $scope.FromDate }
            }).success(function (data) {
                if (data.success) {
    ...
    $scope.GenerateReportForSearchResult = function () {
        if ($scope.ProductEntryHistoryForSpecificDateInSellPointForm.$valid) {
            $http({
                ...
                method: 'POST',
                url: '/ProductEntryHistoryForSpecificDateInSellPoint/GenerateReportForSearchResult/',
                data: { sellsPointStoreId: $scope.SellsPoint.SellsPointStoreId, shiftId: $scope.SellsPoint.ShiftId, fromDate: $scope.FromDate, productList: $scope.ProductList, totalAmount: $scope.totalAmount }
            }).success(function (data) {
                if (data.success) {
                    toastr.success(data.successMessage);
                    $scope.ProductList = data.result;
